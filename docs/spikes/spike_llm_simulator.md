...
| Parse / JSON errors отсутствуют или явно посчитаны | в результате присутствует поле `n_parse_errors`; идеально `= 0`, если `> 0` — эти решения помечены и не включены в метрики переполнения | sanity |
| Skip-rate в разумных пределах | `n_skipped / n_decisions ≤ 0.30` (из `n_decisions` исключены решения, не сделанные из-за `budget_exceeded`) | sanity, parse-fault rate |
...
5. `n_parse_errors` зафиксирован в результате (= 0 идеально, > 0 явно учтено и не включено в метрики переполнения).
6. `n_skipped / n_decisions ≤ 0.30` (без учёта `n_decisions_aborted`).
...
**Все open questions §13 подтверждены пользователем 2026-05-07.** Этап G принят. Переход к этапу H — отдельным сообщением пользователя.
