"""Smoke EC1/EC3 на новой программе Mobius EN с BGE-large-en + ABTT-1.

Сценарии:
- EC1: при capacity_multiplier = 3.0 все 3 политики (П1, П2, П3) дают
  `mean_overload_excess` = 0.
- EC3: при w_rec = 0 все политики дают идентичный `mean_overload_excess`
  (CRN-инвариантность, range == 0).

Цель — подтвердить, что переключение эмбеддера на BGE-large-en + ABTT-1 не
ломает архитектурные инварианты на реальной программе (а не только на toy).

Запуск:
    cd experiments && source .venv/bin/activate
    python scripts/smoke_ec_mobius_en.py
"""
...
CONF_JSON = ROOT / "data/conferences/mobius_2025_autumn_en.json"
CONF_EMB = ROOT / "data/conferences/mobius_2025_autumn_en_embeddings.npz"
PERS_JSON = ROOT / "data/personas/personas_mobius_en.json"
PERS_EMB = ROOT / "data/personas/personas_mobius_en_embeddings.npz"
