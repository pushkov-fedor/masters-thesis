...
Включает full V-прогон LLM-агентского симулятора на английских промптах и
cross-validation Q-O7 (см. §9).
...
| П4 (llm_ranker) | 12 maximin × 3 replicate = 36 evals на gpt-4o-mini, cost $0.29 |
| V (LLM cross-validation) | 12 maximin × 4 политики × 1 seed = **48 evals**, 44 160 LLMAgent-вызовов на gpt-5.4-nano, EN-промпты, cost $10.22, wallclock 2 ч 23 мин |
...
| LHS Q (с П4) | `results/lhs_parametric_mobius_2025_autumn_en_2026-05-12.{json,csv,md}` (486 evals, П4 на 12 maximin) |
| Analyze S | `results/en/analysis_*.json` (8 файлов), `results/en/analysis_lhs_parametric_2026-05-12_en.md`, `results/en/plots/*.png` |
| LLM bilingual templates | `experiments/src/llm_agent.py` — параметризован language=ru/en, `experiments/scripts/run_llm_lhs_subset.py` — флаг `--language` |
| V (LLM agents) | `results/en/llm_agents_lhs_subset_12pts_en.{json,csv,md}` |
| Cross-validation | `results/en/analysis_cross_validation_en.json`, `results/en/analysis_cross_validation_2026-05-12_en.md`, `results/en/plots/cross_validation_rho_per_metric.png` |
...
## 9. Cross-validation (V) — выполнено на EN-пайплайне

LLM-агентский симулятор `gpt-5.4-nano` на английских промптах (паритет каналов),
12 maximin LHS × 4 политики × 1 seed = 48 evals, 44 160 LLMAgent-вызовов, cost
$10.22, wallclock 2 ч 23 мин, 0 timeouts / 0 errors, parse-errors ~0.5%.
LLMRankerPolicy переиспользовала warm cache от Q-rerun (100% hit, $0 LLM-ranker
cost в V).

| Метрика | n_LHS_in_ρ / 12 | median ρ | mean ρ | passed |
|---|---:|---:|---:|---|
| `mean_user_utility` | 12 | **0.67** | 0.65 | PASS |
| `hall_utilization_variance` | 12 | **0.80** | 0.59 | **PASS** |
| `overflow_rate_slothall` | 2 | 0.41 | 0.41 | FAIL (узкая выборка) |
| `mean_overload_excess` | 2 | 0.15 | 0.15 | FAIL (узкая выборка) |

**Объединённая median ρ = 0.769 ≥ 0.5 → Q-O7 PASS** (vs 0.554 в RU-прогоне).

### 9.1. Top-1 match per metric

«Top-1» — политика с лучшим (минимальным) рангом совпала ли у параметрика и
LLM. Колонка `non-degen` фильтрует fake-match через `argmin` tie-breaking на
constant ranks.

| Метрика | match all / n | match non-degen / n_non-degen |
|---|---:|---:|
| `mean_overload_excess` | 9 / 12 | **2 / 2** (100%) |
| `overflow_rate_slothall` | 8 / 12 | 1 / 2 |
| `hall_utilization_variance` | 9 / 12 | **9 / 12** (75%) |
| `mean_user_utility` | 6 / 12 | 6 / 12 (50%) |

### 9.2. Сравнение с RU-прогоном

| Метрика | RU median ρ | EN median ρ | Δ |
|---|---:|---:|---:|
| `mean_user_utility` | 0.80 | 0.67 | −0.13 |
| `overflow_rate_slothall` | 0.74 | 0.41 | −0.33 |
| `mean_overload_excess` | 0.30 | 0.15 | −0.15 |
| `hall_utilization_variance` | 0.40 | **0.80** | **+0.40** |
| **Overall median** | 0.554 | **0.769** | **+0.215** |

Главное изменение — `hall_utilization_variance`: FAIL (0.40) → PASS (0.80) на
полной выборке 12 / 12 LHS. Параметрик и LLM согласованно ранжируют политики
по распределению загрузки залов, чего не было в RU-прогоне.

`mean_user_utility` чуть просел (0.80 → 0.67), но остался выше порога; и
параметрик, и LLM сходятся на том, что utility у политик практически равна
(top-1 match 6 / 12, на оставшихся ranking коллапсирует в ties).

Overload-семейство (`mean_overload_excess`, `overflow_rate_slothall`) остаётся
структурно дегенерированным: 10 из 12 maximin LHS дают overload = 0 у всех
политик в параметрике, ρ пересчитывается только на 2 невырожденных точках. Это
не дефект пайплайна, а свойство Mobius при выборке LHS (74% safe scenarios,
см. §6).

### 9.3. Корректная формулировка результата

LLM cross-validation на EN-пайплайне **усиливает** выводы параметрика по двум
из четырёх метрик (utility и hall_utilization_variance) на полной выборке 12 /
12 LHS-row, и подтверждает направление по overload на узком невырожденном
подмножестве (2 / 12). Это **budget cross-validation** на экономичной модели
`gpt-5.4-nano` ($10.22), не сильная валидация на real attendance.

Это не «LLM подтвердил параметрик полностью» — это правдивая картина согласия
двух источников отклика к одной задаче с одной и той же языковой средой.
...
1. **П4 (llm_ranker) только на 12 maximin** — П4 на full-50 не делается по
   Q-O9 (бюджет, accepted). Любые full-50 таблицы содержат только П1–П3.
2. **Cross-validation overload** на 2 / 12 non-degenerate LHS — статистически
   малорепрезентативно; интерпретация diagnostic, не строгое согласование.
...
- **Cross-validation Q-O7 пройден сильнее, чем в RU**: median ρ = 0.769 (vs
  0.554). `hall_utilization_variance` перешла из FAIL (ρ = 0.40) в PASS (ρ =
  0.80) на полной выборке 12 / 12 LHS. Паритет каналов (EN-промпты + EN-эмбеддинги)
  даёт более согласованное сравнение параметрика и LLM-агента, чем смесь языков
  RU-прогона.
...
- LHS Q (с П4): `lhs_parametric_mobius_2025_autumn_en_2026-05-12.{json,csv,md}`
- Analyze S: `results/en/analysis_*.json` (8 файлов),
  `results/en/analysis_lhs_parametric_2026-05-12_en.md`
- V (LLM agents): `results/en/llm_agents_lhs_subset_12pts_en.{json,csv,md}`
- Cross-validation: `results/en/analysis_cross_validation_en.json`,
  `results/en/analysis_cross_validation_2026-05-12_en.md`
- Plots: `results/en/plots/{analysis_risk_utility_scatter,analysis_gossip_bucket_bar,analysis_ranking_heatmap_maximin,cross_validation_rho_per_metric}.png`
