...
|---|---:|---:|---:|
| контрольная vs по релевантности | 0.02 | 0.94 | 0.04 |
| контрольная vs с учётом вместимости | 0.00 | 0.84 | 0.16 |
| по релевантности vs с учётом вместимости | 0.00 | 0.86 | 0.14 |

</div>

<div class="mt-3 px-3 py-2 rounded bg-bluegreen-50 border border-bluegreen-200 text-sm">

Политика по релевантности не выигрывает у политики с учётом вместимости ни на одной из 50 точек — ни строго, ни в пределах допуска. Обратное направление: политика с учётом вместимости строго лучше за пределами допуска в 14% точек (и в 20% по строгому сравнению без допуска).
...
В 39 из 50 точек гиперкуба переполнения нет ни у одной политики — это структурное свойство равномерного покрытия осей. Эффект политики наблюдается на оставшихся 11 точках:

<div class="text-sm">

| Точка | вмест. | ауд. | по релевантности | с учётом вместимости |
|---:|---:|---:|---:|---:|
| 26 | 0.629 | 60 | 0.071 | 0.012 |
| 49 | 0.774 | 100 | 0.510 | 0.362 |
| 35 | 0.963 | 100 | 0.056 | 0.030 |
| 18 | 1.040 | 60 | 0.002 | 0.000 |

</div>

<div class="mt-3 text-sm">

На рисковом подмножестве политика с учётом вместимости не уступает альтернативам в пределах допуска во всех 11 точках; строго снижает риск перегрузки в 8 точках из 11 (73%).

</div>

<div class="mt-2 text-xs opacity-70">

Различие средней релевантности фактически выбранного доклада между политиками — менее 0.001 по абсолютному значению (политики «по релевантности» и «с учётом вместимости» обе выигрывают у контрольной за пределами допуска).
...
<!--
Это центральный численный результат работы. На полном плане из 50 точек политика по релевантности не выигрывает у политики с учётом вместимости ни в одной точке. На подмножестве из 11 точек, где переполнение вообще зафиксировано, политика с учётом вместимости строго снижает риск в 8 точках. Остальные 39 точек структурно безопасны: при равномерном покрытии оси множителя вместимости значительная часть точек попадает в режим избыточной вместимости, где переполнения не возникает физически. Снижение риска происходит без потери содержательного качества рекомендаций: различие средней релевантности фактически выбранного доклада между политиками — менее 0.001, при этом обе политики выигрывают у контрольной за пределами допуска.
-->
...
|---|---:|---:|---:|
| дисперсия загрузки залов | 12 | **0.80** | 9 / 12 |
| средняя релевантность | 12 | 0.67 | 6 / 12 |
| доля переполнений | 2 | 0.41 | 1 / 2 |
| относительное превышение вместимости | 2 | 0.15 | 2 / 2 |

</div>

<div class="mt-3 text-sm">

Объединённая медианная корреляция Спирмена $\rho = 0.769$, формальный порог 0.5 пройден существенно.
...
Согласованность по дисперсии загрузки залов уверенная: на 12 точках из 12 ранжирования политик в двух симуляторах совпадают, лидер совпадает на 9 точках из 12. Согласованность по средней релевантности также подтверждена на полной выборке 12 / 12.
...
<!--
Перекрёстная проверка проведена на 12 общих точках гиперкуба. Согласованность ранжирований политик по дисперсии загрузки залов уверенная: корреляция 0.80, лидер совпадает на 9 точках из 12 — на полной выборке без вырожденных случаев. Согласованность по средней релевантности также подтверждена на 12 точках из 12, корреляция 0.67. По метрикам переполнения корреляция опирается всего на 2 точки из 12: на остальных 10 параметрический симулятор даёт ноль для всех политик, и ранжирование вырождено. Объединённая медианная корреляция Спирмена 0.77 — порог 0.5 пройден существенно. В литературе по агентским симуляторам на языковых моделях каноном валидации считается совпадение распределений, а не точность предсказания индивидуального выбора (Лароой и Тёрнберг, 2025).
-->
...
На подмножестве из 11 точек с положительным риском перегрузки политика с учётом вместимости строго снижает риск в 8 точках из 11.

</div>

<div class="p-3 rounded bg-bluegreen-50 border border-bluegreen-200 text-sm">

Конфликт между снижением риска перегрузки и потерей содержательного качества рекомендаций возникает в 2% сценарных комбинаций.
